...
- (void)startSession {
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    AVCaptureVideoDataOutput *videoFileOutput = [[AVCaptureVideoDataOutput alloc] init];
    if (!self.isScanning) {
      ...
      [videoFileOutput setVideoSettings:rgbOutputSettings];
      [videoFileOutput setAlwaysDiscardsLateVideoFrames:YES];
      [videoFileOutput setSampleBufferDelegate:self queue:self.dataOutputQueue];
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  if (barcodes.count > 0) {
    NSDictionary *event = @{
                            ...
                            @"bounds": @{
                                @"origin": @{
                                    @"x": [NSString stringWithFormat:@"%f", barcodes[0].bounds.origin.x],
                                    @"y": [NSString stringWithFormat:@"%f", barcodes[0].bounds.origin.y]
                                    },
                                @"size": @{
                                    @"height": [NSString stringWithFormat:@"%f", barcodes[0].bounds.size.height],
                                    @"width": [NSString stringWithFormat:@"%f", barcodes[0].bounds.size.width],
                                    }
                                }
                            };
